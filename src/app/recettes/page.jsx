...
const CATEGORIES = [
  { name: 'Toutes', value: '', emoji: '🍽️' },
  { name: 'Entrées', value: 'Entrées', emoji: '🥗' },
  { name: 'Plats principaux', value: 'Plats principaux', emoji: '🍲' },
  { name: 'Desserts', value: 'Desserts', emoji: '🍰' },
  { name: 'Salades', value: 'Salades', emoji: '🥙' },
  { name: 'Soupes', value: 'Soupes', emoji: '🍜' },
  { name: 'Snacks', value: 'Snacks', emoji: '🍿' },
  { name: 'Boissons', value: 'Boissons', emoji: '🍹' },
  { name: 'Petit-déjeuner', value: 'Petit-déjeuner', emoji: '🥐' },
];

export const metadata = {
  title: 'Toutes les Recettes | RecettesBlog',
  description: 'Découvrez toutes nos délicieuses recettes marocaines et internationales. Des entrées aux desserts, trouvez l\'inspiration pour vos repas.',
};
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-50 via-white to-orange-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center">
            ...
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6">
              Toutes Nos <span className="text-yellow-300">Recettes</span>
            </h1>
            <p className="text-white/90 text-lg md:text-xl max-w-2xl mx-auto mb-10">
              Découvrez des centaines de recettes délicieuses, des entrées aux desserts, 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Categories */}
        <div className="mb-12">
          <h2 className="text-center text-xl font-semibold text-gray-700 mb-6">Filtrer par catégorie</h2>
          <div className="flex flex-wrap gap-3 justify-center">
            {CATEGORIES.map((cat) => (
              <Link
                key={cat.value}
                href={`/recettes${cat.value ? `?category=${cat.value}` : ''}`}
        ...
        <div className="flex items-center justify-between mb-8">
          <p className="text-gray-600">
            <span className="font-bold text-orange-600">{data.pagination?.total || 0}</span> recette(s) trouvée(s)
            {currentCategory && (
              <span className="ml-2 text-gray-500">dans "{currentCategory}"</span>
            )}
          </p>
          {currentCategory && (
            <Link
        ...
        {data.recipes?.length > 0 ? (
          ...
          <div className="text-center py-20 bg-white rounded-3xl shadow-lg">
            ...
            <h3 className="text-2xl font-bold text-gray-800 mb-3">Aucune recette trouvée</h3>
            <p className="text-gray-600 mb-6">Essayez de modifier votre recherche ou explorez nos catégories</p>
            <Link
              href="/recettes"
        ...
        {data.pagination?.pages > 1 && (
          <div className="flex justify-center items-center gap-2 mt-16">
            ...
            {currentPage > 1 && (
              <Link
                href={`/recettes?page=${currentPage - 1}${currentCategory ? `&category=${currentCategory}` : ''}`}
                ...
                .map((page, index, array) => {
                  ...
                  return (
                    <div key={page} className="flex items-center gap-2">
                      ...
                      <Link
                        href={`/recettes?page=${page}${currentCategory ? `&category=${currentCategory}` : ''}`}
